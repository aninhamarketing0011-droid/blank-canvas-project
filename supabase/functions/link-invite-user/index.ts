...
serve(async (req) => {
  ...
  try {
    ...
    if (authError || !user) {
      console.error("Falha ao obter usuário autenticado em link-invite-user:", authError);
      return new Response(
    ...
    if (user.id !== userId) {
      return new Response(
        JSON.stringify({ error: "Usuário autenticado não corresponde ao userId enviado." }),
        {
          status: 403,
          headers: { "Content-Type": "application/json", ...corsHeaders },
        },
